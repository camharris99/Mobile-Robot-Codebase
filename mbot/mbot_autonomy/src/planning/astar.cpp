                                            ...
                                            const SearchParams &params)
{
    ...
    printf("Start Astar\n");
    while (!Q.empty())
    { // if curNode == goalNode -> break
        curNode = Q.pop();
        printf("Current Node x: %i, y: %i",curNode->cell.x,curNode->cell.y);
...
double g_cost(Node *from, Node *goal, const ObstacleDistanceGrid &distances, const SearchParams &params)
{
    ...
    double g_cost = 0;
    double cellDistance = distances(from->cell.x, from->cell.y);
...
std::vector<Node *> expand_node(Node *node, const ObstacleDistanceGrid &distances, const SearchParams &params)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        if (!is_collision(neighbor, distances, params) && distances.isCellInGrid(neighbor->cell.x, neighbor->cell.y))
        {
